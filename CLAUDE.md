...
**Self-hosted fonts + palette overhaul**: landed — the Google Fonts CDN `@import` (silently fell
back to system fonts when offline, and leaked visitor IPs to Google) is replaced by self-hosted
OFL woff2 fonts in a new `fonts/` dir, fetched/reproduced by `scripts/fetch-fonts.py` and copied
into `www/` by `scripts/build-www.mjs` for the Capacitor build; `src/cache-manifest.js` precaches
all 5 files so they're available fully offline post-install. Three role-based CSS custom
properties replace ad hoc font-family declarations: `--font-display` (Bitter, headings),
`--font-ui` (Space Grotesk, body/labels/buttons/inputs), `--font-mono` (Space Mono, restricted to
tide-time + height numerals, for tabular figures). Alongside this, the `--color-panel-*` tokens
changed from near-invisible rgba overlays on the moving body gradient to **solid hex** in both
themes, so the tide table reads as a real opaque card surface; light re-hued to a warm
"chart-paper" look, dark panels now solid navy, borders strengthened, `.day` gained a hairline
box-shadow, all key pairs re-verified WCAG AA. **Maintenance gotcha**: `index.html` has FOUR
palette blocks that must be kept in sync by hand — `:root`, `@media (prefers-color-scheme: dark)
:root`, `:root[data-theme="light"]`, `:root[data-theme="dark"]` — a color change only in one is a
common way to reintroduce a light/dark mismatch.
<!-- END AUTO-MANAGED -->

<!-- AUTO-MANAGED: architecture -->
## Architecture

- `index.html` — UI shell; loads `src/ui.js` via a single `<script type="module">import { init } from "./src/ui.js"; init();</script>` (previously contained the inlined engine + old single-station app; now generalized to any station). Visible heading is "Ireland's Tides" (Task 23; the installed-app name stays "RWB Tides" — see `manifest.webmanifest`/`capacitor.config.json`), footer reads "Made in Roaring Water Bay". Task 19 added a `#view-toggle` (List/Map pill buttons) above `.controls`, wrapped the search fields (now scoped by `#county-filter`, Task 27 — see below) in a new `#list-panel` sub-flex-container (so the toggle can hide/show them as one unit without disturbing `.controls`' own layout), and added a `#map-panel` (hidden by default) holding `#map-svg-container`, a `#map-hint` caption, `.map-zoom-controls` (+/−/reset, added with the pan/zoom map), and a `.map-legend`; all new map CSS reuses existing theme custom properties plus a few dedicated ones added since (`--map-sea`/`--map-land`/`--map-coast`, tuned per light/dark theme), no ad hoc one-off colors. Fonts are self-hosted (see project description above): `@font-face` rules load 5 OFL woff2 files from `./fonts/` (populated by `scripts/fetch-fonts.py`, no CDN dependency), assigned to three role tokens — `--font-display` (Bitter), `--font-ui` (Space Grotesk), `--font-mono` (Space Mono, tide-time/height numerals only). Palette lives in **four** blocks that must be kept in sync by hand: `:root` (light default), `@media (prefers-color-scheme: dark) :root`, `:root[data-theme="light"]`, `:root[data-theme="dark"]`; `--color-panel-*` tokens are solid hex (not rgba) in both themes so panels read as an opaque card surface over the moving body gradient.
...
- `src/cache-manifest.js` — `CACHE_VERSION` + `CACHE_ASSETS` (app-shell precache list) consumed by `sw.js`; `CACHE_VERSION` is rewritten by `scripts/build-data.mjs` on every successful data build, so regenerating the dataset auto-invalidates the runtime data cache; `CACHE_ASSETS` precaches all station indexes (`data/stations.json` TICON + `data/mi-stations.json` MI + `data/epa-stations.json` EPA) plus the search-alias data files (`data/beaches.json`, `data/places.json`) plus, since Task 19, the map picker's coastline outline (`data/ireland-outline.json`) and its two source modules (`src/geo.js`, `src/map.js`), plus (added since) `data/low-water.json` (foreshore overlay), `data/spot-overrides.json` (Task 22 per-spot source overrides), and the 5 self-hosted `./fonts/*.woff2` files (Bitter ×2 weights, Space Grotesk variable, Space Mono ×2 weights) so typography is fully offline post-install
...
- `scripts/build-www.mjs` — packaging-only (not a data build): assembles the offline web app into `www/` (Capacitor's `webDir`) by copying `index.html`, `src/`, `data/`, `manifest.webmanifest`, `sw.js`, `icons/`, and (added since) `fonts/` from repo root, clean each run; GitHub Pages still serves the repo root directly and is untouched by this — `www/` is gitignored and native-build-only. Same "only run when executed directly" guard as `build-data.mjs`/`build-mi.mjs`
- `fonts/` — committed self-hosted OFL woff2 fonts (`bitter-500.woff2`, `bitter-700.woff2`, `space-grotesk-var.woff2`, `space-mono-400.woff2`, `space-mono-700.woff2`), fetched/reproduced by `scripts/fetch-fonts.py`; replaces a prior Google Fonts CDN `@import` that fell back to system fonts offline and leaked visitor IPs to Google. Loaded via `@font-face` in `index.html`, precached by `src/cache-manifest.js`, copied into `www/` by `scripts/build-www.mjs` for the Capacitor build
