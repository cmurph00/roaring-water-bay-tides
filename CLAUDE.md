...
**EPA node predictions (West Cork)**: landed (Task 18), fixed (Task 21) — a third offline
...
model's 10-minute sampling grid — extremes are now refined by parabolic interpolation
...
~12km from any beach) — naming is now purely nearest-registered-bathing-beach within 2km
...
- `data/named-spots.json` — flat, hand-maintained array (no build script) of well-known West Cork place names not on the EPA bathing-water register (`{name, latitude, longitude}` only), currently Baltimore/Schull/Crookhaven/Cape Clear. Same search-only-alias contract as `data/beaches.json` (reused via `searchBeaches()` in `src/ui.js`, same "📍"-prefixed / `wireLocalityClick` handling): resolves at click-time via `nearestStation()` over the merged EPA/MI/TICON `index` to whichever real prediction point is actually closest — an EPA beach-model node if one is close enough and kept, otherwise the nearest MI/TICON gauge, never a dropped offshore EPA node. Exists because `scripts/build-epa.mjs`'s register-proximity naming (below) would never label a node after these towns directly.
- `data/epa-stations.json` + `data/epa/<node>.json` — third offline prediction source: named West Cork EPA/Marine Institute hydrodynamic-model nodes (`source: "epa"`, `chart_datum: "Model MSL"`), each with its own precomputed `tides` extrema tuples (same `[epochMs, heightMetres, "high"|"low"]` shape as MI, fixed 2026–2028 window), CC-BY-4.0; built by `scripts/build-epa.mjs` from the ERDDAP `imiTidePredictionEpa` dataset's continuous `sea_surface_height` output (extracted via `extractExtrema`, not borrowed from a distant real gauge). Every shipped node is named purely by proximity to a registered bathing-water beach (`labelNodeFromRegister`, below) — a node with no register beach within `BEACH_NAME_RADIUS_KM` (2km) is OFFSHORE and dropped entirely at build time rather than mislabelled; Baltimore/Schull/Crookhaven/Cape Clear remain reachable as search aliases via `data/named-spots.json` instead
...
- `scripts/build-epa.mjs` — regenerates `data/epa/` + `data/epa-stations.json` (see above); exports `inBbox`, `parseNodeListCsv`, `extractExtrema` (prominence-filtered high/low extraction off the raw continuous series, `MIN_PROMINENCE` 0.15m), `labelNodeFromRegister(node, beaches, maxKm = BEACH_NAME_RADIUS_KM)` (pure, no I/O — names a node after the nearest `data/beaches.json` entry within `maxKm`, or returns `null` meaning OFFSHORE/drop), `BEACH_NAME_RADIUS_KM` (2km), `BBOX`, `WINDOW_YEARS`; `build()` labels every bbox node via `labelNodeFromRegister` before fetching any series data, so an ERDDAP fetch is never spent on a node that's going to be dropped as offshore. Superseded the older hand-tuned-coordinate `NAMED_SPOTS`/`assignNamedSpots` naming approach — same "only run when executed directly" guard as the other build scripts
